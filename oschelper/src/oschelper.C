...
static PyObject *do_osc(PyObject *self, PyObject *args) {
  ...
  if (!PyArg_ParseTuple(args, "OOOOOddid", &energy_obj, &steps_obj, &density_obj, &U_obj, &MP_obj, &dm21, &dm31, &nuind, &dL)) { 
  ...
  if (U_N != 3 || U_M != 3 || MP_N != 3 || MP_M != 3) {
    ...
    std::stringstream error;
    error << "ERROR BAD MATRIX SIZE, MUST BE 3. U ROW " << U_N << " U COL " << U_M << " MP ROW " << MP_N << " MP COL " << MP_M;
    PyErr_SetString(PyExc_ValueError, error.str().c_str());
  ...
  std::complex<double> Uosc[3][3];
  for (unsigned i_A = 0; i_A < 3; i_A++) {
    for (unsigned j_A = 0; j_A < 3; j_A++) {
      Uosc[i_A][j_A] = *((std::complex<double>*)PyArray_GETPTR2(U, i_A, j_A));
    }
  }

  // zero-initialize output array
  npy_intp nu_dim[2] = {3, energy_size};
  ...
  for (int i = 0; i < density_size; i++) { // Time / Length evolution
    ...
    for (int j = 0; j < energy_size; j++) { // Per neutrino energy
      ...
      std::complex<double> A[3][3]; // Build matter potential
      for (unsigned i_A = 0; i_A < 3; i_A++) {
        for (unsigned j_A = 0; j_A < 3; j_A++) {
	  A[i_A][j_A] = 1.52e-4*Ye*rho*E*(*((std::complex<double>*)PyArray_GETPTR2(MP, i_A, j_A)));
        }
      }
      ...
      std::complex<double> thisnu[3] = {*((std::complex<double>*)PyArray_GETPTR2(nu, 0, j)), 
	                          *((std::complex<double>*)PyArray_GETPTR2(nu, 1, j)),
                                  *((std::complex<double>*)PyArray_GETPTR2(nu, 2, j))};
      std::complex<double> J(0,1);
      std::complex<double> dnu[3] = {0, 0, 0};
      dnu[0] = (-J*Losc/(2*Eosc))*(dm31*Uosc[0][2]*std::conj(Uosc[0][2]) + dm21*Uosc[0][1]*std::conj(Uosc[0][1]) + A[0][0])*thisnu[0] +\
               (-J*Losc/(2*Eosc))*(dm31*Uosc[0][2]*std::conj(Uosc[1][2]) + dm21*Uosc[0][1]*std::conj(Uosc[1][1]) + A[0][1])*thisnu[1] +\
               (-J*Losc/(2*Eosc))*(dm31*Uosc[0][2]*std::conj(Uosc[2][2]) + dm21*Uosc[0][1]*std::conj(Uosc[2][1]) + A[0][2])*thisnu[2];
        
      dnu[1] = (-J*Losc/(2*Eosc))*(dm31*Uosc[1][2]*std::conj(Uosc[0][2]) + dm21*Uosc[1][1]*std::conj(Uosc[0][1]) + A[1][0])*thisnu[0] +\
               (-J*Losc/(2*Eosc))*(dm31*Uosc[1][2]*std::conj(Uosc[1][2]) + dm21*Uosc[1][1]*std::conj(Uosc[1][1]) + A[1][1])*thisnu[1] +\
               (-J*Losc/(2*Eosc))*(dm31*Uosc[1][2]*std::conj(Uosc[2][2]) + dm21*Uosc[1][1]*std::conj(Uosc[2][1]) + A[1][2])*thisnu[2];

        
      dnu[2] = (-J*Losc/(2*Eosc))*(dm31*Uosc[2][2]*std::conj(Uosc[0][2]) + dm21*Uosc[2][1]*std::conj(Uosc[0][1]) + A[2][0])*thisnu[0] +\
               (-J*Losc/(2*Eosc))*(dm31*Uosc[2][2]*std::conj(Uosc[1][2]) + dm21*Uosc[2][1]*std::conj(Uosc[1][1]) + A[2][1])*thisnu[1] +\
               (-J*Losc/(2*Eosc))*(dm31*Uosc[2][2]*std::conj(Uosc[2][2]) + dm21*Uosc[2][1]*std::conj(Uosc[2][1]) + A[2][2])*thisnu[2];
